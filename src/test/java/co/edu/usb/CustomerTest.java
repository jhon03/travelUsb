...
class CustomerTest {
	...
	void searchCustomerByStatusPageAndOrderIdNumber() {
		...
		try {
			...
			pageCustomer = customerService.findByStatusOrderByIdentificationNumberAsc(Constantes.Activo, pageable);
		...
		try {
			Customer customer = customerService.findByEmailIgnoreCase("JUAN@HOTMAIL.COM");
			System.out.println(customer.getName());
		}catch (Exception e){
			System.out.println(e.getMessage());
		}
...
void searchCustomerForRangeDate() throws Exception{
	...
	Calendar fechaInicio = new GregorianCalendar(1990, 3, 29);
